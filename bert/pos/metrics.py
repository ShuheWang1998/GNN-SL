    ...
    def forward(self, pred_sequence_labels, gold_sequence_labels, sequence_mask=None):
        ...
        for item_idx, (pred_label_item, gold_label_item) in enumerate(zip(pred_sequence_labels, gold_sequence_labels)):
            if sequence_mask is not None:
                ...
                new_pred_label_item = []
                new_gold_label_item = []
                for sub_idx, sub_sequence_mask_item in enumerate(sequence_mask_item):
                    if sub_sequence_mask_item == 1:
                        new_pred_label_item.append(self.tags2label[pred_label_item[sub_idx]])
                        new_gold_label_item.append(self.tags2label[gold_label_item[sub_idx]])
            else:
                new_pred_label_item = [self.tags2label[tmp] for tmp in pred_label_item]
                new_gold_label_item = [self.tags2label[tmp] for tmp in gold_label_item]
            pred_label_item = new_pred_label_item[1:-1]
            gold_label_item = new_gold_label_item[1:-1]
            pred_entities = transform_entity_bio_labels_to_spans(pred_label_item)
            gold_entities = transform_entity_bio_labels_to_spans(gold_label_item)
...
def count_confusion_matrix(pred_entities, gold_entities):
    ...
    for span_item in pred_entities:
        if span_item in gold_entities:
            ...
            remove_list.append(span_item)
        else:
            false_positive += 1
    for span_item in remove_list:
        pred_entities.remove(span_item)
...
def transform_entity_bio_labels_to_spans(label_sequence, classes_to_ignore=None):
    """
    Given a sequence of BMES-{entity type} labels, extracts spans.
    """
    spans = []
    ...
    while index < len(label_sequence):
        ...
        if label[0] == "B":
            ...
            while True:
                index += 1
                if index >= len(label_sequence):
                    spans.append((start_cate, (start, index-1)))
                    break
                label = label_sequence[index]
                if label[0] != "I":
                    spans.append((start_cate, (start, index-1)))
