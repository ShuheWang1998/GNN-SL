...
class GNNNER(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        sequence_features, neighbours, sequence_labels, knn_values = batch
        ...
        for batch_graph in graphs:
            ...
            sub_knn_label = sub_knn_label.view(-1, self.datastore.l_size+self.datastore.r_size+1, self.args.gnn_k)[:, self.datastore.l_size, :]
        ...
        argmax_labels = self.postprocess_logits_to_labels(logits, new_knn_values, knn_labels)
    ...
    def postprocess_logits_to_labels(self, logits, knn_values, knn_labels):
        ...
        sim_probs = torch.softmax(knn_values / self.link_temperature, dim=-1) # [bsz, max_seq_len, gnn_k]
        ...
        argmax_labels = torch.argmax(probabilities, 2, keepdim=False) # [bsz, max_seq_len]
