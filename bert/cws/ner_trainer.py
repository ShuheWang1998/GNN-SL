...
from dataset import CWSDataset
...
class CWSTask(pl.LightningModule):
    def __init__(self, args: argparse.Namespace):
        ...
        self.entity_labels = CWSDataset.get_labels(os.path.join(args.data_dir, "ner_labels.txt"))
    ...
    def forward(self, batch, batch_idx):
        input_ids, labels, sub_token_starts = batch
        loss_mask = (input_ids != 0).long()
        batch_size, seq_len = input_ids.shape

        sub_token_starts_mask = torch.zeros_like(loss_mask).bool()
        ...
        loss_mask = loss_mask & sub_token_starts_mask

        attention_mask = (input_ids != 0).long()
        return self.model(input_ids, attention_mask=attention_mask), labels, loss_mask
    ...
    def training_step(self, batch, batch_idx):
        ...
        bert_classifiaction_outputs, labels, loss_mask = self.forward(batch, batch_idx)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        bert_classifiaction_outputs, gold_labels, loss_mask = self.forward(batch, batch_idx)
    ...
    def _load_dataset(self, prefix="test"):
        ...
        dataset = CWSDataset(directory=self.args.data_dir, prefix=prefix,
    ...
    def test_step(self, batch, batch_idx):
        ...
        bert_classifiaction_outputs, gold_labels, sequence_mask = self.forward(batch, batch_idx)
    ...
    def save_predictions_to_file(self, gold_entity_lst, pred_entity_lst, prefix="test"):
        ...
        with open(save_file_path, "w") as f:
            for idx_ in range(len(gold_entity_lst)):
                ...
                tokens = data_items[idx_][0]
                tokens_label = data_items[idx_][1]
                f.write("=!" * 20+"\n")
                f.write(" ".join([tokens[sub_idx]+" "+tokens_label[sub_idx] for sub_idx in range(len(tokens))])+"\n")
...
def evaluate():
    ...
    model = CWSTask.load_from_checkpoint(checkpoint_path=args.checkpoint_path,
