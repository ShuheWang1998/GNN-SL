...
from ner_trainer import CWSTask
...
class Datastore(pl.LightningModule):
    ...
    def test_dataloader(self, ) -> DataLoader:
        ...
        dataset = CWSDataset(directory=self.args.data_dir, prefix=self.args.datastore_sub_set,
    ...
    def test_step(self, batch, batch_idx):
        ...
        return {"features": features.cpu(), "labels": gold_labels.cpu(), "mask": sequence_mask.cpu(), "input_ids": input_ids}
        ...
        for x in outputs:
            ...
            for sub_batch_size_idx in range(sub_batch_size):
                ...
                for idx_ in range(np_features.shape[0]):
                    if idx_ == 0:
                        data_store_key_in_memory[now_cnt, l_size:mid_size , :] = np_features[0:l_size+1, :]
                        data_store_val_in_memory[now_cnt, l_size:mid_size] = labels[0:l_size+1]
    ...
    def extract_features(self, outputs):
        ...
        for x in outputs:
            ...
            for sub_batch_size_idx in range(sub_batch_size):
                ...
                features = torch.masked_select(features, mask).view(-1, hidden_size).cpu()
                np_features = features.numpy().astype(np.float32)
                really_features[now_cnt, :np_features.shape[0], :] = np_features[:]
...
def main():
    ...
    ner_model = CWSTask.load_from_checkpoint(checkpoint_path=args.checkpoint_path,
