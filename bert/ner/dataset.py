...
class NERDataset(Dataset):
    ...
    def __init__(self, directory, prefix, vocab_file, config_path, max_length=512, file_name="char.bmes", lower_case=False, language="en", en_roberta=False):
        ...
        self.data_items = NERDataset._read_conll(data_file_path, directory=directory)
    ...
    @staticmethod
    def _read_conll(input_file, delimiter=" ", directory=None):
        ...
        for idx, data_line in enumerate(datalines):
            ...
            if len(data_line) == 0:
                if (len(cached_token) != 0 and len(cached_label) != 0):
                    if directory[-16:] == "official_conll03":
                        cached_token = truecase_sentence(cached_token)
