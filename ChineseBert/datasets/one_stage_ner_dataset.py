...
class NERDataset(Dataset):
    ...
    def __init__(self, directory, prefix, vocab_file, config_path, feature_info_path, nei_path, max_length=512, search_knn_k=32, file_name="char.bmes", task_name="ner", load_neigh=True):
        """
        Args:
        ...
        if task_name == 'ner':
            ...
            self.data_items = NERDataset._read_cws(data_file_path)
    ...
    def __getitem__(self, idx):
        ...
        for idx_ in range(min(self.max_length-2, len(token_sequence))):
            if idx_ > 0 and (self.idx_to_label[label_sequence[idx_]][0] == 'B' or self.idx_to_label[label_sequence[idx_]][0] == 'S'):
                new_token_sequence += " "
        ...
        if self.load_neigh:
            return input_ids, pinyin_ids, label, torch.LongTensor(self.neighbours[idx]).view(-1)
        else:
