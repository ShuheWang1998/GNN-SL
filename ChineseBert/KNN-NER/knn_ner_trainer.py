...
class KNNNERTask(pl.LightningModule):
    ...
    def test_dataloader(self, ) -> DataLoader:
        ...
        token_num = info["token_sum"]
        l_size = info["l_size"]
        r_size = info["r_size"]
        hidden_size = info["hidden_size"]
        mid_size = l_size + r_size + 1
        keys = np.memmap(os.path.join(self.args.datastore_path, "datastore_features.npy"), 
                              dtype=np.float32,
                              mode="r",
                              shape=(token_num, mid_size, hidden_size))
        
        keys_in_memory = np.zeros((token_num, hidden_size), dtype=np.float32)
        keys_in_memory[:] = keys[:, mid_size//2, :]
        
        self.keys = torch.from_numpy(keys_in_memory)

        values = np.memmap(os.path.join(self.args.datastore_path, "datastore_vals.npy"), 
                                dtype=np.int32,
                                mode="r",
                                shape=(token_num, mid_size))
        vals_in_memory = np.zeros((token_num,), dtype=np.int32)
        vals_in_memory[:] = values[:, mid_size//2]
        
        self.vals = torch.from_numpy(vals_in_memory).long()
