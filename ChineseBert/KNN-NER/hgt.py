...
class GNNNERModel(nn.Module):
    def __init__(
        ...
        super(GNNNERModel, self).__init__()
        self.hgt = HGT(
                      etype2idx=etype2idx,
                      ntype2idx=ntype2idx,
                      in_dim=in_dim,
                      ...
                      use_norm=use_norm, dropout=dropout,
                      two_stream=two_stream, attn_drop=attn_drop,
                    )
        self.add_label = add_label
        if self.add_label:
            self.label_embedding = nn.Embedding(num_embeddings=num_labels, embedding_dim=in_dim)
        self.out_layer = nn.Linear(out_dim, num_labels)
        self.out_layer_ntgt = nn.Linear(out_dim, num_labels)
    
    def forward(self, graph: dgl.DGLHeteroGraph, labels: torch.Tensor = None):
        if self.add_label:
            label_embedding = self.label_embedding(labels)
            graph.nodes["label"].data["h"] = label_embedding
            # graph.nodes["ntgt"].data["h"] = graph.nodes["ntgt"].data["h"] + label_embedding
            #graph.nodes["ntgt"].data["h"] = label_embedding
        out_dic = self.hgt(graph)
        #features = self.out_layer(out_dic["tgt"])
        #return features
        features_tgt = self.out_layer(out_dic["tgt"])
        features_ntgt = self.out_layer_ntgt(out_dic["ntgt"])
        return features_tgt, features_ntgt

    #def forward(self, graphs: List[dgl.DGLHeteroGraph], labels: torch.Tensor = None):
    #    if self.add_label:
    #        for idx_ in range(len(graphs)):
    #            graphs[idx_][-2].nodes["label"].data["h"] = self.label_embedding(graphs[idx_][-1])
    #    batch_graph = dgl.batch([sub_graph[-2] for sub_graph in graphs])
    #    out_dic = self.hgt(batch_graph)
    #    features = self.out_layer(out_dic["tgt"])
    #    return features
...
if __name__ == '__main__':
    ...
    def test_hgt():
        ...
        g = g.to('cuda:0')
        model = HGT(
            etype2idx={"intra": 0, "inter": 1, "label_edge": 2},
            ntype2idx = {"src": 0, "nsrc": 1, "tgt": 2, "ntgt": 3},
            in_dim=hidden_dim,
            hidden_dim=hidden_dim,
            out_dim=hidden_dim,
