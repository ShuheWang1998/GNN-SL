...
class GNNNER(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        sequence_features, neighbours, sequence_labels, sequence_mask = batch
        ...
        features, _ = self.model(batch_graph, labels=batch_labels)
        features_label = torch.masked_select(sequence_labels, sequence_mask).view(-1)
        
        return features, features_label

        # features, features_tgt = self.model(batch_graph, labels=batch_labels)

        # logits = torch.cat((features, features_tgt), dim=0).view(-1, self.num_labels)
        # features_label = torch.masked_select(sequence_labels, sequence_mask)
        # labels = torch.cat((features_label, batch_labels), dim=0).view(-1)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        #logits, labels, sequence_mask = self.forward(batch)
        #loss = self.compute_loss(logits, labels, loss_mask=sequence_mask)
        logits, labels, sequence_mask = self.inference(batch)
    ...
    def validation_epoch_end(self, outputs):
        ...
        #self.result_logger.info(f"EVAL INFO -> valid_f1 is: {f1}, loss is: {avg_loss}")
        ...
        tensorboard_logs = {"val_f1": f1, }
        return {"val_log": tensorboard_logs, "val_f1": f1, "val_precision": precision, "val_recall": recall}
...
def main():
    ...
    #path_to_best_checkpoint = "/nfs1/shuhe/gnn_ner/results/ud1.4_gnn_32_label_real_layer2/checkpoint/epoch=3.ckpt"
